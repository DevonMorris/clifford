...
//!
//! ```bash
//! cargo run -p clifford-viz --bin demo_launcher_three_d --features three-d --release
//! ```
//!
//! ## Building for web
//!
...
fn get_demo_name() -> String {
    ...
    {
        std::env::args().nth(1).unwrap_or_else(|| "menu".to_string())
    }
}
...
fn render_menu_ui(ctx: &egui::Context) {
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            render_demo_category(ui, "Projective Geometry (PGA)", &[
                ("projective2", "2D Point-Line", "Join/meet operations in 2D PGA"),
                ("projective2_robot", "Robot Arm", "Forward kinematics with motors"),
            ]);

            ui.add_space(12.0 * sp);

            render_demo_category(ui, "Conformal Geometry (CGA)", &[
                ("conformal2_circles", "Circle from 3 Points", "Create circles via wedge product"),
                ("conformal2_inversion", "Circle Inversion", "Conformal transformation"),
                ("conformal2_mobius", "Mobius Transforms", "Compose transformations"),
                ("conformal2_intersection", "Circle Intersection", "Find intersection points"),
            ]);

            ui.add_space(12.0 * sp);

            render_demo_category(ui, "Minkowski Spacetime", &[
                ("minkowski2_diagram", "1+1D Diagram", "Light cones and Lorentz boosts"),
                ("minkowski2_dilation", "Time Dilation", "The geometry of time dilation"),
            ]);

            ui.add_space(12.0 * sp);

            render_demo_category(ui, "Complex & Dual Numbers", &[
...
fn navigate_to_demo(demo_id: &str) {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        eprintln!("To run: cargo run -p clifford-viz --bin demo_launcher_three_d --features three-d -- {}", demo_id);
    }
}
