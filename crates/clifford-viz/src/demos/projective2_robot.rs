...
impl RobotArmDemo {
    ...
    fn render_arm(
        ...
        plot_ui.line(
            labeled_line_segment(base.0, base.1, elbow.0, elbow.1, line(ctx), "Link 1").width(4.0),
        );

        // Link 2: Elbow to End Effector (slightly thinner, secondary color)
        plot_ui.line(
            labeled_line_segment(elbow.0, elbow.1, end.0, end.1, motor(ctx), "Link 2").width(3.5),
        );
    }
    ...
    fn render_joints(
        ...
        plot_ui.line(circle_2d(base.0, base.1, 0.15, point(ctx), 32));
        plot_ui.points(
        ...
        // Elbow joint
        plot_ui.line(circle_2d(elbow.0, elbow.1, 0.12, rotor(ctx), 32));
        plot_ui.points(
    ...
    fn render_workspace(&self, ctx: &egui::Context, plot_ui: &mut egui_plot::PlotUi) {
        ...
        plot_ui.line(
            circle_2d(0.0, 0.0, r_max, with_alpha(circle(ctx), 80), 64).name("Outer workspace"),
        );

        // Inner unreachable boundary (if links differ in length)
        if r_min > 0.01 {
            plot_ui.line(
                circle_2d(0.0, 0.0, r_min, with_alpha(line_secondary(ctx), 60), 64)
                    .name("Inner boundary"),
            );
        }
    }
    ...
    fn render_trail(&self, ctx: &egui::Context, plot_ui: &mut egui_plot::PlotUi) {
        if self.trail.len() < 2 {
            return;
        }
        ...
        plot_ui.line(
            egui_plot::Line::new(egui_plot::PlotPoints::new(points))
                .color(with_alpha(selected(ctx), 100))
                .width(1.5)
                .name("Trail"),
        );
    }
...
impl VisualizationApp for RobotArmDemo {
        ...
        Plot::new("robot_arm_plot")
            ...
            .show(ui, |plot_ui| {
                ...
                if self.show_workspace {
                    self.render_workspace(&ctx, plot_ui);
                }

                // End effector trail
                if self.show_trail {
                    self.render_trail(&ctx, plot_ui);
                }

                // Robot arm links
                self.render_arm(&ctx, plot_ui, base, elbow, end);

                // Joint angle arcs
                if self.show_angles {
                    self.render_angle_arcs(&ctx, plot_ui, base, elbow);
                }

                // Joints and end effector
                self.render_joints(&ctx, plot_ui, base, elbow, end);
                ...
                for i in 0..5 {
                    ...
                    plot_ui.line(line_segment(x, -0.15, x - 0.1, -0.25, grid_major(&ctx)));
                }
            });
    }
    ...
    fn controls(&mut self, ui: &mut egui::Ui) {
        ...
        angle_slider_range(ui, "Base theta_1", &mut self.theta1, -180.0, 180.0);
        angle_slider_range(ui, "Elbow theta_2", &mut self.theta2, -180.0, 180.0);
...
Base joint at origin: P_0 = (0, 0)

Elbow position:
    L_1 = (link1_length, 0)  // Link 1 in local coords
    M_1 = Motor::from_rotation(theta_1)
    Elbow = M_1.transform(L_1)

End effector position:
    L_2 = (link2_length, 0)  // Link 2 in local coords
    M_2 = Motor::from_rotation(theta_1 + theta_2)  // Total rotation
    End = Elbow + M_2.transform(L_2)

MOTOR FORMULA for rotation around origin:
    M = cos(theta/2) - sin(theta/2)*e_3

The antisandwich product M^-1PM transforms point P.

WORKSPACE is the annular region between:
    r_max = L_1 + L_2  (fully extended)
    r_min = |L_1 - L_2|  (fully folded)

Any point in this region is reachable by some (theta_1, theta_2).",

    how_to_use: "\
- JOINT SLIDERS: Drag theta_1 and theta_2 to control the arm
...
or set theta_2 to +-180 deg to fold the arm back on itself.",

    key_concepts: "\
- Motors encode rotation via antisandwich product
- Sequential transformation follows kinematic chain
- Joint angles are relative: theta_2 is relative to link 1
- Workspace = annular region of reachable positions
- Forward kinematics: (theta_1, theta_2) -> (x, y)
- Inverse kinematics: (x, y) -> (theta_1, theta_2) (not covered)",
