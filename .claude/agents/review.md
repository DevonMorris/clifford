...
- [ ] Arbitrary impls follow generic pattern:
  ...
  - Constrained types use factory methods, not random coefficients
...
**Constrained types** (Motor, Flector, Line with Plücker condition) must use **factory-based generation**, not random coefficients:

- [ ] **Factory methods used** - Arbitrary generates via `from_rotation()`, `from_translation()`, `compose()`, etc.
- [ ] **No random coefficients for constrained types** - Random values passed to `new_unchecked()` violate constraints
- [ ] **Generic wrappers used** - Uses `Unit<T>`, `Bulk<T>`, `Ideal<T>` from `crate::wrappers`, not bespoke types
- [ ] **Type aliases used in tests** - Uses `BulkMotor<f64>` not `Bulk<Motor<f64>>` for readability

**Red flags in Arbitrary implementations:**
```rust
// FLAG: Random coefficients don't satisfy Study condition!
impl<T: Float + Debug> Arbitrary for Motor<T> {
    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        (
            -10.0f64..10.0, -10.0f64..10.0, ...  // 8 random values
        )
...
// FLAG: Bespoke wrapper type - use Bulk<Motor<T>> instead
pub struct UnitMotor<T: Float>(pub Motor<T>);

// OK: Factory-based generation guarantees constraints
impl<T: Float + Debug> Arbitrary for Motor<T> {
    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        (
            -std::f64::consts::PI..std::f64::consts::PI,  // angle
            -100.0f64..100.0, -100.0f64..100.0, -100.0f64..100.0,  // translation
        )
            .prop_map(|(angle, tx, ty, tz)| {
                let rotation = Motor::from_rotation_z(T::from_f64(angle));
                let translation = Motor::from_translation(...);
                translation.compose(&rotation)  // Valid motor!
            })
            .boxed()
    }
}
...
**When Arbitrary uses random coefficients:**
- For unconstrained types (Point, Plane, Vector) - OK
- For constrained types (Motor, Flector, constrained Lines) - NOT OK, use factory methods
