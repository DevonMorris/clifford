...
impl<T: Float> Hyperbolic<T> {
    ...
    #[doc = "Creates a Hyperbolic from 1 independent components, computing `hyp`.\n\nReturns `None` if the divisor would be zero (unstable computation)."]
    #[inline]
    pub fn from_components(real: T) -> Option<Self> {
        if (real).abs() < T::epsilon() {
            return None;
        }
        Some(Self::new_unchecked(real, (T::zero()) / (real)))
    }
...
#[doc = "A bulk-normalized HypUnit (bulk norm = 1).\n\nThis type alias wraps HypUnit in `Bulk<T>`, which normalizes by the bulk (non-degenerate) part of the norm."]
pub type BulkHypUnit<T> = crate::wrappers::Bulk<HypUnit<T>>;
#[doc = "A unitized HypUnit (weight norm = 1).\n\nThis type alias wraps HypUnit in `Unitized<T>`, which normalizes by the weight (degenerate) part of the norm."]
pub type UnitizedHypUnit<T> = crate::wrappers::Unitized<HypUnit<T>>;
#[doc = "A unit HypUnit (Euclidean norm = 1).\n\nThis type alias provides compile-time documentation that the HypUnit has been normalized."]
pub type UnitHypUnit<T> = crate::wrappers::Unit<HypUnit<T>>;
#[doc = "A bulk-normalized Hyperbolic (bulk norm = 1).\n\nThis type alias wraps Hyperbolic in `Bulk<T>`, which normalizes by the bulk (non-degenerate) part of the norm."]
pub type BulkHyperbolic<T> = crate::wrappers::Bulk<Hyperbolic<T>>;
#[doc = "A unitized Hyperbolic (weight norm = 1).\n\nThis type alias wraps Hyperbolic in `Unitized<T>`, which normalizes by the weight (degenerate) part of the norm."]
pub type UnitizedHyperbolic<T> = crate::wrappers::Unitized<Hyperbolic<T>>;
