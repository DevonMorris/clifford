...
impl AbsDiffEq for Bivec3<f32> {
    ...
    fn abs_diff_eq(&self, other: &Self, epsilon: Self::Epsilon) -> bool {
        f32::abs_diff_eq(&self.xy, &other.xy, epsilon)
            && f32::abs_diff_eq(&self.xz, &other.xz, epsilon)
            && f32::abs_diff_eq(&self.yz, &other.yz, epsilon)
    }
}

impl RelativeEq for Bivec3<f32> {
    fn default_max_relative() -> Self::Epsilon {
        f32::default_max_relative()
    }
    ...
    fn relative_eq(
        ...
        f32::relative_eq(&self.xy, &other.xy, epsilon, max_relative)
            && f32::relative_eq(&self.xz, &other.xz, epsilon, max_relative)
            && f32::relative_eq(&self.yz, &other.yz, epsilon, max_relative)
    }
}

impl UlpsEq for Bivec3<f32> {
    fn default_max_ulps() -> u32 {
        f32::default_max_ulps()
    }
    ...
    fn ulps_eq(&self, other: &Self, epsilon: Self::Epsilon, max_ulps: u32) -> bool {
        f32::ulps_eq(&self.xy, &other.xy, epsilon, max_ulps)
            && f32::ulps_eq(&self.xz, &other.xz, epsilon, max_ulps)
            && f32::ulps_eq(&self.yz, &other.yz, epsilon, max_ulps)
    }
}

impl AbsDiffEq for Trivec3<f32> {
    type Epsilon = f32;
...
impl RelativeEq for Trivec3<f32> {
    fn default_max_relative() -> Self::Epsilon {
        f32::default_max_relative()
    }
...
impl RelativeEq for Rotor3<f32> {
    fn default_max_relative() -> Self::Epsilon {
        f32::default_max_relative()
    }
...
impl AbsDiffEq for Bivec3<f64> {
    ...
    fn abs_diff_eq(&self, other: &Self, epsilon: Self::Epsilon) -> bool {
        f64::abs_diff_eq(&self.xy, &other.xy, epsilon)
            && f64::abs_diff_eq(&self.xz, &other.xz, epsilon)
            && f64::abs_diff_eq(&self.yz, &other.yz, epsilon)
    }
}

impl RelativeEq for Bivec3<f64> {
    fn default_max_relative() -> Self::Epsilon {
        f64::default_max_relative()
    }
    ...
    fn relative_eq(
        ...
        f64::relative_eq(&self.xy, &other.xy, epsilon, max_relative)
            && f64::relative_eq(&self.xz, &other.xz, epsilon, max_relative)
            && f64::relative_eq(&self.yz, &other.yz, epsilon, max_relative)
    }
}

impl UlpsEq for Bivec3<f64> {
    fn default_max_ulps() -> u32 {
        f64::default_max_ulps()
    }
    ...
    fn ulps_eq(&self, other: &Self, epsilon: Self::Epsilon, max_ulps: u32) -> bool {
        f64::ulps_eq(&self.xy, &other.xy, epsilon, max_ulps)
            && f64::ulps_eq(&self.xz, &other.xz, epsilon, max_ulps)
            && f64::ulps_eq(&self.yz, &other.yz, epsilon, max_ulps)
    }
}

impl AbsDiffEq for Trivec3<f64> {
    type Epsilon = f64;
...
impl RelativeEq for Trivec3<f64> {
    fn default_max_relative() -> Self::Epsilon {
        f64::default_max_relative()
    }
...
impl RelativeEq for Rotor3<f64> {
    fn default_max_relative() -> Self::Epsilon {
        f64::default_max_relative()
    }
