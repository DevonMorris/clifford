...
pub fn sandwich_rotor_vector<T: Float>(v: &Rotor<T>, x: &Vector<T>) -> Vector<T> {
    Vector::new(
        ...
        v.s() * x.y() * v.s()
            ...
            + v.s() * x.z() * v.yz()
            - v.xy() * x.x() * v.s(),
        -(v.xz() * x.z() * v.xz()) + v.yz() * x.x() * v.xy() - v.xy() * x.y() * v.xz()
            + v.xy() * x.x() * v.yz()
            - v.xz() * x.x() * v.s()
            - v.yz() * x.z() * v.yz()
            - v.yz() * x.y() * v.s()
...
pub fn sandwich_rotor_bivector<T: Float>(v: &Rotor<T>, x: &Bivector<T>) -> Bivector<T> {
    Bivector::new(
        v.s() * x.xz() * v.yz() - v.s() * x.yz() * v.xz() - v.xz() * x.xy() * v.xz()
            + v.yz() * x.xz() * v.s()
            + v.xy() * x.yz() * v.yz()
            + v.s() * x.xy() * v.s()
            + v.xz() * x.xz() * v.xy()
            ...
            + v.xy() * x.xy() * v.xy()
            + v.xy() * x.xz() * v.xz(),
        v.xy() * x.yz() * v.s() - v.yz() * x.xy() * v.s() - v.xy() * x.xz() * v.xy()
            + v.yz() * x.yz() * v.xz()
            - v.s() * x.xy() * v.yz()
            + v.xz() * x.yz() * v.yz()
            + v.xy() * x.xy() * v.xz()
...
pub fn sandwich_rotor_rotor<T: Float>(v: &Rotor<T>, x: &Rotor<T>) -> Rotor<T> {
    Rotor::new_unchecked(
        v.xz() * x.s() * v.xz()
            ...
            - v.xz() * x.xz() * v.s()
            - v.xy() * x.xz() * v.yz(),
        -(v.yz() * x.s() * v.xz()) + v.s() * x.xy() * v.s() - v.s() * x.s() * v.xy()
            + v.yz() * x.xz() * v.s()
            + v.xy() * x.s() * v.s()
            + v.xy() * x.xy() * v.xy()
            + v.xy() * x.xz() * v.xz()
